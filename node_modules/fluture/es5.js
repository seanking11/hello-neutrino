...
(function (global, f) {
  ...
  Future.encase2 = function Future$encase2(f, x, y) {
    switch (arguments.length) {
      case 1:
        return unaryPartial(Future$encase2, f);
      case 2:
        return binaryPartial(Future$encase2, f, x);
      default:
  ...
  Future.encase3 = function Future$encase3(f, x, y, z) {
    switch (arguments.length) {
      case 1:
        return unaryPartial(Future$encase3, f);
      case 2:
        return binaryPartial(Future$encase3, f, x);
      case 3:
        return ternaryPartial(Future$encase3, f, x, y);
      default:
  ...
  Future.fromPromise2 = function Future$fromPromise2(f, x, y) {
    switch (arguments.length) {
      case 1:
        return unaryPartial(Future$fromPromise2, f);
      case 2:
        return binaryPartial(Future$fromPromise2, f, x);
      default:
  ...
  Future.fromPromise3 = function Future$fromPromise3(f, x, y, z) {
    switch (arguments.length) {
      case 1:
        return unaryPartial(Future$fromPromise3, f);
      case 2:
        return binaryPartial(Future$fromPromise3, f, x);
      case 3:
        return ternaryPartial(Future$fromPromise3, f, x, y);
      default:
  ...
  SafeFuture.prototype._f = function SafeFuture$fork(rej, res) {
    var open = true;
    var f = this._computation(function SafeFuture$fork$rej(x) {
      if (open) {
        open = false;
        rej(x);
      }
  ...
  function check$chainRec$f(m, f, i, x) {
    if (!isFuture(m)) throw new TypeError('Future.chainRec expects the function its given to return a Future every' + ' time it is called. The value returned from' + (ordinal[i] ? ' the ' + ordinal[i] + ' call' : ' call ' + i) + ' was not a Future.' + ('\n  Actual: ' + show(m) + '\n  From calling: ' + showf(f) + '\n  With: (Next, Done, ' + show(x) + ')'));
  }

  function check$chainRec$it(it, i) {
    if (!isIteration(it)) throw new TypeError('Future.chainRec expects the function its given to return a Future of an' + ' Iteration every time it is called. The Future returned from' + (ordinal[i] ? ' the ' + ordinal[i] + ' call' : ' call ' + i) + ' did not resolve to a member of Iteration.' + '\n  You can create an uncomplete or complete Iteration using the next' + ' or done functions respectively. These are passed into your callback' + ' as first and second arguments.' + ('\n  Actual: Future.of(' + show(it) + ')'));
  }
  ...
  ChainRec.prototype._f = function ChainRec$fork(rej, res) {
    ...
    (function Future$chainRec$recur(state) {
      ...
      while (!state.done) {
        timing = Undetermined;
        var m = _this._iterate(Next, Done, state.value);
  ...
  CachedFuture.prototype.run = function CachedFuture$run() {
    ...
    _this._cancel = _this._pure._f(function CachedFuture$fork$rej(x) {
      _this.reject(x);
    }, function CachedFuture$fork$res(x) {
      _this.resolve(x);
    });
  };
  ...
  CachedFuture.prototype._f = function CachedFuture$fork(rej, res) {
    ...
    switch (_this._state) {
      case 1:
        cancel = _this._addToQueue(rej, res);break;
      case 2:
        rej(_this._value);break;
      case 3:
        res(_this._value);break;
      default:
        cancel = _this._addToQueue(rej, res);_this.run();
    }
    return cancel;
  };
  ...
  FutureNode.prototype._f = function FutureNode$fork(rej, res) {
    ...
    return function FutureNode$fork$cancel() {
      open = false;
    };
  };
  ...
  FutureAfter.prototype._f = function FutureAfter$fork(rej, res) {
    ...
    return function FutureAfter$fork$cancel() {
      clearTimeout(id);
    };
  };
  ...
  FutureRejectAfter.prototype._f = function FutureRejectAfter$fork(rej) {
    ...
    return function FutureRejectAfter$fork$cancel() {
      clearTimeout(id);
    };
  };
  ...
  function check$parallel$m(m, i) {
    if (!isFuture(m)) throw new TypeError('Future.parallel expects its second argument to be an array of Futures.' + (' The value at position ' + i + ' in the array was not a Future\n  Actual: ' + show(m)));
  }
  ...
  FutureParallel.prototype._f = function FutureParallel$fork(rej, res) {
    var _this = this,
        cancels = new Array(_this._max),
        out = new Array(_this._length);
    var i = _this._max,
        ok = 0;
    var cancelAll = function Future$parallel$cancel() {
      for (var n = 0; n < _this._max; n++) {
        cancels[n] && cancels[n]();
      }
    };
    var run = function FutureParallel$fork$run(future, j, c) {
      ...
      cancels[c] = future._f(function Future$parallel$fork$rej(reason) {
        ...
        out[j] = value;
        ok = ok + 1;
        if (i < _this._length) run(_this._futures[i], i++, c);else if (ok === _this._length) res(out);
      });
    };
    for (var n = 0; n < _this._max; n++) {
      run(_this._futures[n], n, n);
    }return cancelAll;
  };
  ...
  FutureDo.prototype._f = function FutureDo$fork(rej, res) {
    ...
    check$do$g(iterator);
    var recurser = new ChainRec(function Future$do$next(next, _, x) {
  ...
  FutureEncase.prototype.toString = function FutureEncase$toString() {
    var args = [this._a, this._b, this._c].slice(0, this._length).map(show).join(', ');
    var name = 'encase' + (this._length > 1 ? this._length : '');
  ...
  function check$fromPromise$p(p, f, a, b, c) {
    if (!isThenable(p)) throw new TypeError('Future.fromPromise expects the function its given to return a Promise/Thenable' + ('\n  Actual: ' + show(p) + '\n  From calling: ' + showf(f)) + ('\n  With a: ' + show(a)) + (arguments.length > 3 ? '\n  With b: ' + show(b) : '') + (arguments.length > 4 ? '\n  With c: ' + show(c) : ''));
  }
  ...
  FutureChain.prototype._f = function FutureChain$fork(rej, res) {
    var _this = this;
    var cancel = void 0;
    var r = _this._parent._f(rej, function FutureChain$fork$res(x) {
      var m = _this._chainer(x);
      check$chain$f(m, _this._chainer, x);
      cancel = m._f(rej, res);
    });
    return cancel || (cancel = r, function FutureChain$fork$cancel() {
      cancel();
    });
  };
  ...
  FutureChainRej.prototype._f = function FutureChainRej$fork(rej, res) {
    var _this = this;
    var cancel = void 0;
    var r = _this._parent._f(function FutureChainRej$fork$rej(x) {
      var m = _this._chainer(x);
      check$chainRej$f(m, _this._chainer, x);
      cancel = m._f(rej, res);
    }, res);
    return cancel || (cancel = r, function FutureChainRej$fork$cancel() {
      cancel();
    });
  };
  ...
  function check$ap$f(f) {
    if (!isFunction(f)) throw new TypeError('Future#ap expects its first argument to be a Future of a Function' + ('\n  Actual: Future.of(' + show(f) + ')'));
  }
  ...
  FutureAp.prototype._f = function FutureAp$fork(rej, res) {
    var _this = this;
    var cancel = void 0;
    var r = _this._mval._f(rej, function FutureAp$fork$res$x(x) {
    ...
    return cancel || (cancel = r, function FutureAp$fork$cancel() {
      cancel();
    });
  };
  ...
  FutureRace.prototype._f = function FutureRace$fork(rej, res) {
    var cancelled = false,
        lcancel = noop,
        rcancel = noop;
    var cancel = function FutureRace$fork$cancel() {
      cancelled = true;lcancel();rcancel();
    };
    var reject = function FutureRace$fork$rej(x) {
      cancel();rej(x);
    };
    var resolve = function FutureRace$fork$res(x) {
      cancel();res(x);
    };
  ...
  FutureAnd.prototype._f = function FutureAnd$fork(rej, res) {
    var rejected = false,
        ...
        lcancel = noop,
        rcancel = noop;
    lcancel = this._left._f(function (e) {
      rejected = true;rcancel();rej(e);
    }, function (_) {
      return rejected ? rej(val) : resolved ? res(val) : resolved = true;
    });
    rcancel = this._right._f(function (e) {
      return resolved ? rej(e) : (rejected = true, val = e);
    }, function (x) {
      return resolved ? res(x) : (resolved = true, val = x);
    });
    return function FutureAnd$fork$cancel() {
      lcancel();rcancel();
    };
  };
  ...
  FutureOr.prototype._f = function FutureOr$fork(rej, res) {
    var resolved = false,
        ...
        lcancel = noop,
        rcancel = noop;
    lcancel = this._left._f(function (_) {
      return rejected ? rej(err) : resolved ? res(val) : rejected = true;
    }, function (x) {
      resolved = true;rcancel();res(x);
    });
    resolved || (rcancel = this._right._f(function (e) {
      return resolved || (rejected ? rej(e) : (err = e, rejected = true));
    }, function (x) {
      return resolved || (rejected ? res(x) : (val = x, resolved = true));
    }));
    return function FutureOr$fork$cancel() {
      lcancel();rcancel();
    };
  };
  ...
  FutureBoth.prototype._f = function FutureBoth$fork(rej, res) {
    ...
    lcancel = this._left._f(function FutureBoth$fork$rejLeft(e) {
      ...
      tuple[0] = x;
      if (resolved) res(tuple);else resolved = true;
    });
    ...
    return function FutureBoth$fork$cancel() {
      lcancel();rcancel();
    };
  };
  ...
  FutureHook.prototype._f = function FutureHook$fork(rej, res) {
    ...
    cancelAcquire = _this._acquire._f(rej, function FutureHook$fork$res(resource) {
      var disposer = function FutureHook$dispose(callback) {
        var disposal = _this._dispose(resource);
        check$hook$f(disposal, _this._dispose, resource);
        cancel = disposal._f(rej, callback);
        return cancel;
      };
      var consumption = _this._consume(resource);
      check$hook$g(consumption, _this._consume, resource);
      var cancelConsume = consumption._f(function (x) {
        return disposer(function (_) {
          return rej(x);
        });
      }, function (x) {
        return disposer(function (_) {
    ...
    return function FutureHook$fork$cancel() {
      cancel();
    };
  };
  ...
  Future.classes = {
    ...
    FutureOf: FutureOf,
    FutureReject: FutureReject,
    FutureRejectAfter: FutureRejectAfter,
    FutureNode: FutureNode,
    FutureAfter: FutureAfter,
    FutureParallel: FutureParallel,
    FutureDo: FutureDo,
    FutureTry: FutureTry,
    FutureEncase: FutureEncase,
    FutureFromPromise: FutureFromPromise,
    FutureChain: FutureChain,
    FutureChainRej: FutureChainRej,
    FutureMap: FutureMap,
    FutureMapRej: FutureMapRej,
    FutureBimap: FutureBimap,
    FutureAp: FutureAp,
    ...
    FutureAnd: FutureAnd,
    FutureOr: FutureOr,
    FutureBoth: FutureBoth,
    FutureFold: FutureFold,
